...
from mv_dts import RegressMusicVideoDataset
...
class TSProj(torch.nn.Module):
    def __init__(self):
        ...
        self.classifier = torch.nn.Linear(4096, 2, bias=False)
        self.regressor = torch.nn.Linear(4096, 1, bias=False)
        torch.nn.init.zeros_(self.regressor.weight)

    def forward(self, x):
        return dict(
            logits=self.classifier(x),
            scores=torch.nn.functional.sigmoid(self.regressor(x))
        )


class VicunaLLaVANeXTVideoDPO(torch.nn.Module):
    def __init__(self, frames, peft_mode, lora_rank):
        ...
        question = (
            DEFAULT_IMAGE_TOKEN + "\n" +
            f"Focusing on the feelings and atmosphere from the visuals of the given video, tell me which part most possibly indicate a shift in atmosphere and why? Give me a short answer."
        )
        answer = "It's [TS]."
    ...
    def forward(self, frames, t_labels, t_scores, device="cuda"):
        b = frames.shape[0]

        (_attention_masks, _inputs_embeds, _labels) = self.__prepare_inputs_labels_for_multimodal(frames, device)

        t_labels = t_labels.to(device=device).long()
        t_scores = t_scores.to(device=device).float()
        ...
        t_results = self.t_proj(embeds)
        pred_logits = t_results["logits"].float()
        pred_scores = t_results["scores"].float()
    ...
    def generate(self, frames, device="cuda"):
        b = frames.shape[0]
        ...
        for i, token_id in enumerate(output_ids[0]):
            if token_id == self.SEG_TOKEN_IDX:
                ...
                t_results = self.t_proj(ts_embed)
                t_probs = t_results["logits"].float().softmax(dim=-1)
                t_scores = t_results["scores"].float()
                break
        else:
            t_probs = torch.ones((b, 2)) / 2
            t_scores = torch.ones((b, 1)) * 0.5
            miss = True

        return t_probs, t_scores, miss
...
def main():
    ...
    def _run_evaluation_loop(in_training=False):
        ...
        with torch.inference_mode():
            ...
            for i, batch in pbar:
                ...
                t_probs, t_scores, miss = model.generate(
                    batch["frames"],
                    device=model_engine.device
                )

                t_labels = batch["labels"].long()
                t_norms = batch["norms"].float()
                ...
                eval_norms.extend(t_norms.tolist())
                eval_probs.extend(t_probs.tolist())
                eval_scores.extend(t_scores.flatten().tolist())

            eval_results = dict(
                eval_probs=eval_probs,
                eval_norms=eval_norms,
        ...
        if (GLOBAL_RANK == 0):
            ...
            eval_norms = np.concatenate([results["eval_norms"] for results in gathered_results], axis=0)
            eval_scores = np.concatenate([results["eval_scores"] for results in gathered_results], axis=0)
            ...
            eval_labels_bool = eval_labels.astype(bool)
            eval_mse_loss = np.square(eval_scores[eval_labels_bool] - eval_norms[eval_labels_bool]).mean()

            events = [
                ("Eval/Accuracy", eval_accuracy, model_engine.global_samples),
                # ("Eval/CLSLoss", eval_cls_loss, model_engine.global_samples),
                ("Eval/MSELoss", eval_mse_loss, model_engine.global_samples),
    ...
    def _run_training_loop():
        ...
        for epoch in range(args.epochs):
            ...
            for i, batch in pbar:
                losses = model_engine(batch["frames"], batch["labels"], batch["norms"], device=model_engine.device)

                all_loss = sum([losses[k] for k in losses])

                model_engine.backward(all_loss)
                model_engine.step()

                pbar.set_description(f"Loss: {all_loss:.2f} ({', '.join([f'{k}:{losses[k]:.2f}' for k in losses])}), Epoch {epoch+1}")
