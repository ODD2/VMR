...
from mv_dts import FullVideoDataset
...
def evaluation(model, enum_iter, device="cuda"):
    ...
    for i, batch in enum_iter:
        ...
        if (response is None):
            eval_miss_responses += 1

        eval_responses.append(response)
        eval_meta_paths.append(batch["meta_path"][0])

    return dict(
        eval_responses=eval_responses,
        eval_meta_paths=eval_meta_paths,
        eval_miss_responses=eval_miss_responses
    )
...
def main():
    ...
    class Model(torch.nn.Module):
        def __init__(self):
            ...
            question = (
                DEFAULT_IMAGE_TOKEN + "\n" +
                "Think step by step, watch frame by frame. Does the given video present a story just like a short movie? " +
                "Give me a short answer. The format should be 'Yes' or 'No'."
            )
    ...
    def _single_shot_eval():
        ...
        if (GLOBAL_RANK == 0):
            eval_responses = [output for results in gathered_results for output in results["eval_responses"]]
            eval_meta_paths = [output for results in gathered_results for output in results["eval_meta_paths"]]
            eval_miss_responses = sum([results["eval_miss_responses"] for results in gathered_results])

            # print("Missing:", eval_miss_responses)
            # for response, meta_path in zip(eval_responses, eval_meta_paths):
            #     print(response, meta_path)

            with open("check_story.pickle", "wb") as f:
                pickle.dump(
                    dict(
                        eval_responses=eval_responses,
                        eval_meta_paths=eval_meta_paths,
